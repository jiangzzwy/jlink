...
package com.jlink.flink.security;
...
public class SecurityConfiguration {
    ...
    public static final ConfigOption<List<String>> SECURITY_CONTEXT_FACTORY_CLASSES =
            key("security.context.factory.classes")
                    ...
                    .asList()
                    .defaultValues(
                            "com.jlink.flink.security.contexts.HadoopSecurityContextFactory",
                            "com.jlink.flink.security.contexts.NoOpSecurityContextFactory")
                    .withDescription(
    ...
    public static final ConfigOption<List<String>> SECURITY_MODULE_FACTORY_CLASSES =
            key("security.module.factory.classes")
                    ...
                    .asList()
                    .defaultValues(
                            "com.jlink.flink.security.modules.HadoopModuleFactory",
                            "com.jlink.flink.security.modules.JaasModuleFactory",
                            "com.jlink.flink.security.modules.ZookeeperModuleFactory")
                    .withDescription(
     ...
     */
    public SecurityConfiguration(Configuration flinkConf) throws MalformedURLException {
        this(
                flinkConf,
    ...
    public SecurityConfiguration(
            ...
            List<String> securityModuleFactories) throws MalformedURLException {
        this.isZkSaslDisable = flinkConf.getBoolean(SecurityOptions.ZOOKEEPER_SASL_DISABLE);

        this.kdc = flinkConf.getString(CustomOptions.CUSTOM_KERBEROS_KDC);
        this.realm = flinkConf.getString(CustomOptions.CUSTOM_KERBEROS_REALM);

//        this.keytab = flinkConf.getString(SecurityOptions.KERBEROS_LOGIN_KEYTAB);
        if (null != flinkConf.get(SecurityOptions.KERBEROS_LOGIN_KEYTAB)) {
            this.keytab = FlinkConfigUtils.hadoopKeyTabPath(new URL(flinkConf.getString(SecurityOptions.KERBEROS_LOGIN_KEYTAB)));
    ...
    private void validate() {
        if (!StringUtils.isBlank(keytab)) {
            ...
            if (StringUtils.isBlank(kdc)) {
                throw new IllegalConfigurationException(
                        "Kerberos login configuration is invalid: kdc server requires a kdc.");
            }
            if (StringUtils.isBlank(realm)) {
                throw new IllegalConfigurationException(
                        "Kerberos login configuration is invalid: kdc server requires a realm.");
            }
