...
import com.jlink.flink.client.deployment.AbstractContainerizedClusterClientFactory;
...
public class YarnClusterClientFactory
    ...
    private YarnClusterDescriptor getClusterDescriptor(Configuration configuration) throws ClassNotFoundException {
        final YarnClient yarnClient = YarnClient.createYarnClient();
        final YarnConfiguration yarnConfiguration = new YarnConfiguration();
        configuration.get(CustomOptions.CUSTOM_HADOOP_FILES)
                .stream()
                .map(url -> {
                    try {
                        return new URL(url);
                ...
                .filter(url -> url != null)
                .forEach(url -> yarnConfiguration.addResource(url));
