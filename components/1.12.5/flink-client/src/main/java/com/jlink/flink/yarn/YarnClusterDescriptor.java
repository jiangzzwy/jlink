...
import com.jlink.flink.client.deployment.ClusterDeploymentException;
import com.jlink.flink.client.deployment.ClusterRetrieveException;
...
import com.jlink.flink.plugins.PluginUtils;
import com.jlink.flink.yarn.entrypoint.YarnApplicationClusterEntryPoint;
import com.jlink.flink.yarn.entrypoint.YarnJobClusterEntrypoint;
import com.jlink.flink.yarn.entrypoint.YarnSessionClusterEntrypoint;
...
import com.jlink.flink.client.deployment.application.ApplicationConfiguration;
import com.jlink.flink.client.program.ClusterClientProvider;
import com.jlink.flink.client.program.rest.RestClusterClient;
...
import com.jlink.flink.yarn.configuration.YarnDeploymentTarget;
import com.jlink.flink.yarn.configuration.YarnLogConfigUtil;
...
import static com.jlink.flink.yarn.YarnConfigKeys.LOCAL_RESOURCE_DESCRIPTOR_SEPARATOR;
...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
    ...
    public YarnClusterDescriptor(
            ...
            boolean sharedYarnClient) throws ClassNotFoundException {
    ...
    private Optional<Path> getLocalFlinkDistPath(final Configuration configuration) throws ClassNotFoundException {
    ...
    private String getDecodedJarPath() throws ClassNotFoundException {
        final String encodedJarPath =
                Class.forName("org.apache.flink.yarn.YarnClusterDescriptor").getProtectionDomain().getCodeSource().getLocation().getPath();
        try {
    ...
    private ClusterClientProvider<ApplicationId> deployInternal(
        ...
        CustomOptions.getCustomOptions().stream().forEach(co->flinkConfiguration.removeConfig(co));
    ...
    private ApplicationReport startAppMaster(
        ...
        final String callBackUrl = configuration.getString(CustomOptions.CUSTOM_CALLBACK_URL);
        ...
        //yarnConfiguration.set("fs.file.impl", org.apache.hadoop.fs.LocalFileSystem.class.getName());
        ...
        if (HighAvailabilityMode.isHighAvailabilityModeActivated(configuration)) {
            ...
            appContext.setMaxAppAttempts(Integer.valueOf(configuration.getOptional(YarnConfigOptions.APPLICATION_ATTEMPTS).orElseGet(()->"1")));
        }
        ...
        // 添加需要依赖的jar位置
        systemShipFiles.addAll(
                jobGraph.getClasspaths().stream()
                        .map(f -> new File(f.getPath()))
                        .collect(Collectors.toSet()));

        //添加当前依赖jar所在包的位置信息
        final String agentPath = this.getClass().getProtectionDomain().getCodeSource().
                getLocation().toURI().getPath();
        systemShipFiles.add(new File(agentPath));
        LOG.info("Add agent path {} successfully!",agentPath);
        ...
        try {
            ...
            //去除底层回调URL
            if(configuration.containsKey(CustomOptions.CUSTOM_CALLBACK_URL.key())){
                configuration.removeConfig(CustomOptions.CUSTOM_CALLBACK_URL);
            }

            BootstrapTools.writeConfiguration(configuration, tmpConfigurationFile);
            //放回去callbackurl
            if(configuration.containsKey(CustomOptions.CUSTOM_CALLBACK_URL.key())) {
                configuration.setString(CustomOptions.CUSTOM_CALLBACK_URL, callBackUrl);
            }
        ...
        while (true) {
            ...
            deployResult.setState(appState.name());
            ...
            switch (appState) {
                ...
                case KILLED:
                    // 将结果数据回调 应用服务
                    if(callBackUrl!=null){
                        FlinkStatusUtils.callBack(callBackUrl, DeployResultHolder.get());
                    }
                    throw new YarnDeploymentException(
                ...
                default:
                    ...
                    if (System.currentTimeMillis() - startTime > 1 * 60000) {
                        ...
                        //判断调用时间，如果超多等待时长，则自动抛出异常
                        final long currentTimeMillis = System.currentTimeMillis();
                        System.out.println("调用等待时长:"+((currentTimeMillis-startTime)/1000.0)+"秒");
                        if(currentTimeMillis-startTime > 1000 * 90  && callBackUrl==null){
                            killCluster(appId);
                            throw  new RuntimeException("已经等待90秒,请检查服务器资源");
                        }
                    }
                    // 将结果数据回调 应用服务
                    if(callBackUrl!=null){
    ...
    @VisibleForTesting
    void addLibFoldersToShipFiles(Collection<File> effectiveShipFiles) throws URISyntaxException {
        ...
        String libDir=getFlinkConfiguration().getString(DeploymentOptionsInternal.CONF_DIR)+"/../lib";
        if (libDir != null) {
    ...
    void addPluginsFoldersToShipFiles(Collection<File> effectiveShipFiles) {
        String pluginsDirStr=getFlinkConfiguration().getString(DeploymentOptionsInternal.CONF_DIR)+"/../plugins";
        final Optional<File> pluginsDir = Optional.of(new File(pluginsDirStr)) ;
